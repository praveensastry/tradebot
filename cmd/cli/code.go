...
var codeInitCmd = &cobra.Command{
	Use:     `init [name]`,
	Short:   "initialize tradebot code repo",
	Example: `  code init demo-app --version=v1.2.0 --git-user=praveensastry`,
	RunE:    runCodeInit,
}

func init() {
	codeInitCmd.Flags().StringVar(&codeGitUser, "git-user", "", "GitHub user or org")
	codeInitCmd.Flags().StringVar(&codeVersion, "version", "master", "tradebot repo tag or branch name")
...
func runCodeInit(cmd *cobra.Command, args []string) error {
	...
	tmpPath := "/tmp/k8s-tradebot"
	versionName := fmt.Sprintf("k8s-tradebot-%s", codeVersion)

	downloadURL := fmt.Sprintf("https://github.com/praveensastry/tradebot/archive/%s.zip", codeVersion)
	...
	pkgFrom := "github.com/praveensastry/tradebot"
	...
	projFrom := "praveensastry/tradebot"
